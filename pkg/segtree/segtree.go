...
// UpdateFunc receive data of left-child node and right-child node
// calculate and return the current node data
// it can be implement by user and pass as param when creating segment tree
type UpdateFunc func(leftChildData, rightChildData interface{}) (currentNodeData interface{})
...
// NewTree .
func NewTree(nodes []Node, updateFunc UpdateFunc) *Tree {
	t := &Tree{
		updateFunc: updateFunc,
		leafCount:  len(nodes),
		mutex:      sync.Mutex{},
...
func build(nodes []Node, updateFunc UpdateFunc) (data []interface{}, rangeStart, rangeEnd map[int]uint64) {
	if len(nodes) == 0 {
		return nil, nil, nil
	}
	count := len(nodes)
